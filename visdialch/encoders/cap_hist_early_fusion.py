...
class CaptionHistoryEarlyFusionEncoder(nn.Module):
    ...
    def forward(self, batch, cfq_batch=False, cfic_batch=False):
        ...
        if cfic_batch:
            ...
            # cap, cap_len = cap[index_cfic], cap_len[index_cfic]
            img = img[index_cfic]
        ...
        img_ques_mcan_embedding = self.img_ques_fusion(
            image_features,
            ques_embed_per_word,
            lang_ques_feat_mask
        )
        ...
        # shape (batch_size * num_rounds, lstm_hidden_size)
        hist_ques_mcan_embedding = self.hist_ques_fusion(
            hist_embed_per_word,
            ques_embed_per_word,
            lang_ques_feat_mask
        )
