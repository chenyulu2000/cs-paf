    ...
    for epoch in range(start_epoch, end_epoch + 1):
        ...
        for i, batch in enumerate(tqdm(combined_dataloader)):
            ...
            if finetune:
                ...
                if opt.cfq_interval != -1 and i % opt.cfq_interval == 1:
                    is_cfq = True
                elif opt.cfic_interval != -1 and i % opt.cfic_interval == 1:
                    is_cfic = True
                ...
                if is_cfq:
                    ...
                    cfq_loss = criterion_loss(opt=opt, batch=batch, criterion=criterion,
                                              output=cfq_output)
                    batch_loss += opt.lambda_cfq * cfq_loss
                if is_cfic:
                    cfic_output = model(batch, False, True)
                    batch_loss += opt.lambda_cfic * criterion_loss(opt=opt, batch=batch, criterion=criterion,
                                                                   output=cfic_output)
                batch_loss = relu(batch_loss + opt.margin_value)
